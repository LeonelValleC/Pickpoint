...
namespace Voyager_SN
{
    public partial class Reports : Form
    {
        ...
        {
            if (!Directory.Exists(@"C:/VoyagerFile/"))
            {
                Directory.CreateDirectory(@"C:/VoyagerFile/");


                using (System.IO.FileStream fs = new System.IO.FileStream("C:\\VoyagerFile\\Voyager SN.xlsx", FileMode.OpenOrCreate, FileAccess.Write, FileShare.Write))
                {
                    byte[] data = Properties.Resources.Voyager_SN;
        ...
        private void EditExcel()
        {
            ...
            Workbook workbook = excel.Workbooks.Open(@"C:\\VoyagerFile\Voyager SN.xlsx", ReadOnly: false, Editable: true);
        ...
        private void Search()
        {
            ...
            if (cb_Filter.Text == "Voyager")
            {
                ...
                {
                    wo.Id_wo = int.Parse(wo.ReturnID("select id_wo from tb_WO where wo = '" + txt_Search.Text + "'"));
                    salida_datos = "select wo.wo as 'wono', concat(inp.SerialNumber,',',pn.pn, '-',pn.rev  ) as 'serialno', pn.pn as 'part_no', pn.rev as 'revision' from tb_Inprocess inp join tb_WO wo on inp.id_wo = wo.id_wo join tb_PN pn on wo.id_pn = pn.id_pn join tb_User usr on  usr.id_user = inp.id_user where wo.id_wo = '" + wo.Id_wo + "' and inp.Validated = 1 and inp.Scrap is null order by inp.id_inprocess asc";
                }
